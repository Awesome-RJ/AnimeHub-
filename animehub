...
def echo_thread(update, context):
    ...
    if last_command == "recommend":
        ...
        if len(anime_list) == 0:
            ...
            for anime in anime_list:
                markup = [[InlineKeyboardButton(
                    "Recommendations ", callback_data="r=" + anime["session"])]]
        ...
        if len(anime_list) == 0:
            ...
            for anime in anime_list:
                try:
                    markup = [[InlineKeyboardButton(
                        "Episodes", callback_data="d=" + anime["href"])]]
                    context.bot.send_photo(
        ...
        if len(anime_list) == 0:
            ...
            for anime in anime_list:
                markup = [[InlineKeyboardButton(
                    "Anime Info", callback_data="i=" + anime["session"])]]
        ...
        try:
            ...
            if len(recommendations) == 0:
                ...
                for i in recommendations:
                    markup = [[InlineKeyboardButton(
                        "Get Anime Info", callback_data="i=" + i["session"])]]
...
def start(update, context):
    ...
    markup = ReplyKeyboardMarkup([[KeyboardButton("/download"), KeyboardButton("/recommend"), KeyboardButton("/latest")], [
                                 KeyboardButton("/info"), KeyboardButton("/about"), KeyboardButton("/help")]], resize_keyboard=True)
    context.bot.send_message(
...
abour_handler = CommandHandler("about", about)
